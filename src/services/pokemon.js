...
export const showAllItems = url => dispatch => {
...
export const filterItemsByType = url => dispatch => {
  dispatch(setLoading(true));
  fetchItemsFromServer(url)
  .then(data => getPokemonsByType(data.pokemon))
  .then(data => dispatch(setItems(data)))
  .then(() => dispatch(setLoading(false)))
}

export const getEvolutionChain = id => dispatch => {
  fetchItemsFromServer(POKEMON_SPECIES + id)
  .then(data => fetchItemsFromServer(data.evolution_chain.url))
  .then(data => {
...
export const getMoves = pokemon => pokemon.moves.map(move => move.move.name
  .toLowerCase()
  .split('-')
